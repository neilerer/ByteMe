...
def merge_path_dict(d_1, d_2):
	...
	while len(d_1) > 0 and len(d_2) > 0:
		...
		key_1, key_2 = next(iter(d_1)), next(iter(d_2))
		# value of interest
		value_1, value_2 = len(d_1[key_1][0]), len(d_2[key_2][0])
		# put the lowest time in first
		if value_1 < value_2:
			d_return[key_1] = d_1[key_1]
			del d_1[key_1]
...
def merge_sort_path_dict(d_input):
	d_return = copy.deepcopy(d_input)
	# objects
	d_1 = {}
	d_2 = {}
	...
	if len(d_return) > 1:
		...
		for i in range(middle, length, 1):
			...
			del d_return[next_item]
		# recursion
		d_1 = merge_sort_path_dict(d_1)
		d_2 = merge_sort_path_dict(d_2)
		# sorting
		d_return = merge_path_dict(d_1, d_2)
	# return
	return d_return