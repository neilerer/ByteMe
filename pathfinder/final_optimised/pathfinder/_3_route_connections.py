...
def route_connections(route_list, ctt_dict, weekday, time_unit, end):
	...
	if len(route_list) < 1:
		...
		for stop in ctt_dict[weekday][time_unit][route_list[0]]:
			if (route_list[0], route_list[0]) not in route_connections_dict:
				route_connections_dict[(route_list[0], route_list[0])] = [stop]
			else:
				route_connections_dict[(route_list[0], route_list[0])].append(stop)
			# don't need to check past the destination
			if stop == end:
				return route_connections_dict
	else:
		for i in range(1, len(route_list), 1):
			for stop in ctt_dict[weekday][time_unit][route_list[i - 1]]:
				for other_stop in ctt_dict[weekday][time_unit][route_list[i]]:
					if stop == other_stop:
						if (route_list[i - 1], route_list[i]) not in route_connections_dict:
							route_connections_dict[(route_list[i - 1], route_list[i])] = [stop]
						else:
							route_connections_dict[(route_list[i - 1], route_list[i])].append(stop)
						# don't need to check past the destination
						if stop == end:
							return route_connections_dict
	return route_connections_dict
