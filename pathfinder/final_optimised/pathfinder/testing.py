...
def dijkstra(stop_dict, weekday, time_unit, start_stop, end_stop):
	...
	for stop_tuple in stop_dict[weekday][time_unit][start_stop]:
		journey_id += 1
		# [time, route, route:(start, next, time, route)]
		path_dict[journey_id] = [stop_tuple[2], stop_tuple[3], {stop_tuple[3]: [stop_tuple]}]
	...
	while not found:
		...
		current_route = current_shortest[1]
		current_details = current_shortest[2][current_route][-1]
		# details
		...
		if current_stop == end_stop:
			...
			for stop_tuple in stop_dict[weekday][time_unit][current_stop]:
				journey_id += 1
				# new route not visited
				if stop_tuple[3] != current_route and stop_tuple[3] not in current_shortest[2]:
					new_path = current_shortest[2]
					new_path[stop_tuple[3]] = [(stop_tuple[0], stop_tuple[1], stop_tuple[2] + 300, stop_tuple[3])]
					path_dict[journey_id] = [current_shortest_time + stop_tuple[2] + 300, stop_tuple[3], new_path]
				# same route
				elif stop_tuple[3] == current_route:
					new_path = current_shortest[2]
					new_path[current_route].append(stop_tuple)
					path_dict[journey_id] = [current_shortest_time + stop_tuple[2], current_route, new_path]
