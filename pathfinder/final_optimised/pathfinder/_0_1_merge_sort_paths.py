...
def merge_sort(d_input):
	d_return = merge_sort_transfers(d_input)
	return d_return



def merge_on_time_key(d_1, d_2):
	"""
	This functions merges two dictionaries (each of which are assumed to be sorted as desired)
	"""
	# create the return object 
...
def merge_sort_time_key(d_input):
	d_return = copy.deepcopy(d_input)
	# objects
	d_1 = {}
	d_2 = {}
	...
	if len(d_return) > 1:
		...
		for i in range(middle, length, 1):
			...
			del d_return[next_item]
		# recursion
		d_1 = merge_sort_time_key(d_1)
		d_2 = merge_sort_time_key(d_2)
		# sorting
		d_return = merge_on_time_key(d_1, d_2)
	# return
	return d_return