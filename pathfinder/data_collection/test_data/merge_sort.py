...
def merge_dict_for_generation(d_1, d_2):
	"""
	...
	while len(d_1) > 0 and len(d_2) > 0:
		...
		# put the later stop into the return dictionary
		if d_1[key_1][1] > d_2[key_2][1]:
...
def merge_sort_for_generation(d_input):
	# objects
	...
	if len(d_return) > 1:
		...
		for i in range(middle, length, 1):
			...
			del d_return[next_item]
		# recursion
		d_1 = merge_sort_for_generation(d_1)
		d_2 = merge_sort_for_generation(d_2)
		# sorting
		d_return = merge_dict_for_generation(d_1, d_2)
	# return
	return d_return